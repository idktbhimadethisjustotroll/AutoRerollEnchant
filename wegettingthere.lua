...
local function debugButtonSearch()
    ...
    for _, obj in pairs(game.Players.LocalPlayer.PlayerGui:GetDescendants()) do
        if obj:IsA("TextButton") then
            -- Log all buttons found in the UI with details
...
for _, obj in pairs(game.Players.LocalPlayer.PlayerGui:GetDescendants()) do
    if obj:IsA("TextButton") then
        -- Exclude irrelevant buttons based on their names
        if obj.Name:match("cover") or obj.Name:match("toggle") or obj.Name:match("button") or obj.Name:match("backing") then
            print("Skipping irrelevant button: " .. obj.Name)
            continue  -- Skip irrelevant buttons
        end
        
        -- Log button properties to debug
        print("Checking Button - Name: " .. obj.Name .. " | Text: " .. obj.Text .. " | ClassName: " .. obj.ClassName)
        
        -- Check if button text contains "Reroll" and a price (like 750)
...
if rerollButton then
    ...
    -- If not found, log all buttons to help debug
    debugButtonSearch()
    warn("Could not find 'Reroll All' button!")
    return
end

-- Function to check the current enchant
local function checkEnchant()
    -- You should replace this with the correct method to get the current enchant
    enchantText = game:GetService("PlayerGui"):FindFirstChild("EnchantLabel")  -- Adjust this path as necessary
    if enchantText then
